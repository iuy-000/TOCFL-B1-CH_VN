...
@app.route("/")
def index():
    return render_template("index.html", page_mode="public")

@app.route("/teacher/login")
def teacher_login():
    return render_template("teacher_login_google.html", page_mode="public")
...
def teacher_dashboard():
    ...
    conn.close(); return render_template("teacher_dashboard.html", class_data=class_data, page_mode="teacher")
...
def student_enter():
    ...
    return render_template("student_enter.html", page_mode="public")
...
def student_dashboard():
    ...
    return render_template("student_dashboard_practice.html", me=me, leaderboard=leaderboard, unit_familiarity=unit_familiarity, units_count=len(UNITS), page_mode="student")
...
def record_practice():
    ...
    best_streak=p['best_streak'] or 0
    sign_days=p['sign_days'] or 0
    last_checkin=p['last_checkin']
    sign_in_awarded=False
    if score >= 80 and last != today:
        current_streak = current_streak+1 if last and (today-last).days==1 else 1
        best_streak = max(best_streak, current_streak)
        sign_days += 1
        last_checkin = datetime.utcnow().isoformat()
        sign_in_awarded = True

    cur.execute('UPDATE progress SET weekly_points=?, total_points=?, completed_packs=?, accuracy=?, unit_familiarity_json=?, current_streak=?, best_streak=?, sign_days=?, last_checkin=? WHERE student_id=?',
                (weekly_points,total_points,completed_packs,new_acc,json.dumps(unit_data, ensure_ascii=False),current_streak,best_streak,sign_days,last_checkin,student_row_id))
    conn.commit(); conn.close()
    return jsonify({'ok':True,'weekly_points':weekly_points,'completed_packs':completed_packs,'accuracy':new_acc,'unit_familiarity':unit_data.get(unit_no,0), 'familiarity_gain': familiarity_gain, 'score': score, 'sign_days': sign_days, 'sign_in_awarded': sign_in_awarded})
